...
if ! target_parent="$(osascript <<'EOF'
POSIX path of (choose folder with prompt "Waehle den Zielordner fuer das neue Projekt:")
EOF
)"; then
  echo "ERROR: target folder selection was cancelled or failed." >&2
  exit 1
fi
