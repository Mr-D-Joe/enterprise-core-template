...
check_contains "$BASE_DIR/AGENTS.md" "security/runtime/error-contract/migration/version-drift defects unresolved" "No unresolved runtime/security/error/migration/version defects rule in AGENTS present"
check_contains "$BASE_DIR/AGENTS.md" "secure runtime defaults, error-contract boundary, runtime-contract consistency, dependency/supply-chain risk evidence, migration strategy, and version-source consistency" "AUDIT explicit runtime/security/error/migration/version review rule in AGENTS present"
check_contains "$BASE_DIR/AGENTS.md" "Unsichere Runtime-Defaults, Exception-Leaks an Clients, oder stilles Error-Masking" "Explicit prohibition of insecure defaults/exception leaks/silent masking in AGENTS present"
...
check_contains "$BASE_DIR/CONTRIBUTING.md" "secure runtime defaults are verified for production runtime profile" "Secure runtime defaults criterion in CONTRIBUTING present"
check_contains "$BASE_DIR/CONTRIBUTING.md" "no raw internal exception details are exposed in client-facing contracts" "No exception leak criterion in CONTRIBUTING present"
check_contains "$BASE_DIR/CONTRIBUTING.md" "no silent error masking returns success-shaped fallback values for failure states" "No silent masking criterion in CONTRIBUTING present"
check_contains "$BASE_DIR/CONTRIBUTING.md" "runtime contract is consistent (port/interpreter/start command/env keys)" "Runtime contract consistency criterion in CONTRIBUTING present"
check_contains "$BASE_DIR/CONTRIBUTING.md" "dependency vulnerability and supply-chain evidence is linked" "Dependency/supply-chain evidence criterion in CONTRIBUTING present"
check_contains "$BASE_DIR/CONTRIBUTING.md" "persistence changes include schema versioning and migration strategy" "Migration strategy criterion in CONTRIBUTING present"
check_contains "$BASE_DIR/CONTRIBUTING.md" "version sources are synchronized to one canonical source of truth" "Version source consistency criterion in CONTRIBUTING present"
...
check_contains "$BASE_DIR/CONTRIBUTING.md" "Unsichere Runtime-Defaults." "Insecure runtime defaults blocker in CONTRIBUTING present"
check_contains "$BASE_DIR/CONTRIBUTING.md" "Exception-Leak." "Exception leak blocker in CONTRIBUTING present"
check_contains "$BASE_DIR/CONTRIBUTING.md" "Error-Masking." "Silent masking blocker in CONTRIBUTING present"
check_contains "$BASE_DIR/CONTRIBUTING.md" "Runtime-Widerspruch." "Runtime contradiction blocker in CONTRIBUTING present"
check_contains "$BASE_DIR/CONTRIBUTING.md" "Fehlende Dependency-Sicherheitsprüfung." "Missing dependency security check blocker in CONTRIBUTING present"
check_contains "$BASE_DIR/CONTRIBUTING.md" "Persistenz ohne Migrationen." "Persistence without migrations blocker in CONTRIBUTING present"
check_contains "$BASE_DIR/CONTRIBUTING.md" "Versionsdrift." "Version drift blocker in CONTRIBUTING present"
...
check_contains "$BASE_DIR/DESIGN.md" "GOV-41 Secure Runtime Defaults" "Design secure runtime defaults control present"
check_contains "$BASE_DIR/DESIGN.md" "GOV-42 Error Disclosure Boundary" "Design error disclosure boundary control present"
check_contains "$BASE_DIR/DESIGN.md" "GOV-43 No Silent Error Masking" "Design no silent error masking control present"
check_contains "$BASE_DIR/DESIGN.md" "GOV-44 Canonical Runtime Contract" "Design canonical runtime contract control present"
check_contains "$BASE_DIR/DESIGN.md" "GOV-45 Dependency and Supply-Chain Baseline" "Design dependency/supply-chain baseline control present"
check_contains "$BASE_DIR/DESIGN.md" "GOV-46 Persistence Schema Versioning and Migration Strategy" "Design migration strategy control present"
check_contains "$BASE_DIR/DESIGN.md" "GOV-47 Version Single Source of Truth" "Design version single-source control present"
...
check_contains "$BASE_DIR/PROMPTS.md" "secure runtime defaults are verified for release scope" "DEV secure-runtime-defaults completion rule in PROMPTS present"
check_contains "$BASE_DIR/PROMPTS.md" "no raw internal exceptions are exposed in client-facing responses" "DEV no-exception-leak completion rule in PROMPTS present"
check_contains "$BASE_DIR/PROMPTS.md" "no silent error masking reports failure paths as success values" "DEV no-silent-masking completion rule in PROMPTS present"
check_contains "$BASE_DIR/PROMPTS.md" "runtime contract is consistent (port/interpreter/start command/env keys)" "DEV runtime-contract consistency completion rule in PROMPTS present"
check_contains "$BASE_DIR/PROMPTS.md" "dependency/supply-chain vulnerability evidence exists for release scope" "DEV dependency/supply-chain completion rule in PROMPTS present"
check_contains "$BASE_DIR/PROMPTS.md" "persistence changes include schema versioning and migration strategy" "DEV migration strategy completion rule in PROMPTS present"
check_contains "$BASE_DIR/PROMPTS.md" "version source-of-truth is consistent across manifest/build/release artifacts" "DEV version-source consistency completion rule in PROMPTS present"
...
check_contains "$BASE_DIR/PROMPTS.md" 'insecure runtime defaults => `FINAL_STATUS=FAIL`.' "Insecure runtime defaults fail semantics in PROMPTS present"
check_contains "$BASE_DIR/PROMPTS.md" 'raw exception exposure => `FINAL_STATUS=FAIL`.' "Raw exception exposure fail semantics in PROMPTS present"
check_contains "$BASE_DIR/PROMPTS.md" 'silent masking => `FINAL_STATUS=FAIL`.' "Silent masking fail semantics in PROMPTS present"
check_contains "$BASE_DIR/PROMPTS.md" 'runtime contract inconsistent => `FINAL_STATUS=FAIL`.' "Runtime contract inconsistency fail semantics in PROMPTS present"
check_contains "$BASE_DIR/PROMPTS.md" 'missing dependency evidence => `FINAL_STATUS=FAIL`.' "Missing dependency evidence fail semantics in PROMPTS present"
check_contains "$BASE_DIR/PROMPTS.md" 'persistence without migrations => `FINAL_STATUS=FAIL`.' "Persistence without migrations fail semantics in PROMPTS present"
check_contains "$BASE_DIR/PROMPTS.md" 'version source mismatch => `FINAL_STATUS=FAIL`.' "Version-source mismatch fail semantics in PROMPTS present"
