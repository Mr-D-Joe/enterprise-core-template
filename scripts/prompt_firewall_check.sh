...
check_contains "$PROMPTS_FILE" "secure runtime defaults are verified for release scope" "DEV secure-runtime-defaults completion rule present"
check_contains "$PROMPTS_FILE" "no raw internal exceptions are exposed in client-facing responses" "DEV no-exception-leak completion rule present"
check_contains "$PROMPTS_FILE" "no silent error masking reports failure paths as success values" "DEV no-silent-masking completion rule present"
check_contains "$PROMPTS_FILE" "runtime contract is consistent (port/interpreter/start command/env keys)" "DEV runtime-contract consistency completion rule present"
check_contains "$PROMPTS_FILE" "dependency/supply-chain vulnerability evidence exists for release scope" "DEV dependency/supply-chain completion rule present"
check_contains "$PROMPTS_FILE" "persistence changes include schema versioning and migration strategy" "DEV migration-strategy completion rule present"
check_contains "$PROMPTS_FILE" "version source-of-truth is consistent across manifest/build/release artifacts" "DEV version-source consistency completion rule present"
...
check_contains "$PROMPTS_FILE" 'raw exception exposure => `FINAL_STATUS=FAIL`.' "Raw exception exposure fail semantics present"
check_contains "$PROMPTS_FILE" 'silent masking => `FINAL_STATUS=FAIL`.' "Silent masking fail semantics present"
check_contains "$PROMPTS_FILE" 'runtime contract inconsistent => `FINAL_STATUS=FAIL`.' "Runtime contract inconsistency fail semantics present"
check_contains "$PROMPTS_FILE" 'missing dependency evidence => `FINAL_STATUS=FAIL`.' "Missing dependency evidence fail semantics present"
check_contains "$PROMPTS_FILE" 'persistence without migrations => `FINAL_STATUS=FAIL`.' "Persistence without migrations fail semantics present"
check_contains "$PROMPTS_FILE" 'version source mismatch => `FINAL_STATUS=FAIL`.' "Version-source mismatch fail semantics present"
